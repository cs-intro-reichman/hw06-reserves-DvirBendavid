...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		Color[][] t = read("eyes.ppm");
		Color[][] b = read("escher.ppm");
		setCanvas(t);
		//b= scaled(b, t[0].length,t.length);
		//display(blend(b,t, 0.9));
		morph(t,b,5);
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < flip.length ; i++) {
			...
			for (int j = flip[0].length-1; j >=0; j--) {
				flip[i][j] = image[i][col];
				col++;
			}
			col = 0;
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < scaled.length ; i++) {
			...
			for (int j = 0; j <scaled[i].length; j++) {
				scaled[i][j] =image[i*hOrigin/height][j*wOrigin/width];
			}
		}
		return scaled;
	}
